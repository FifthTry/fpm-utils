...
pub mod guest_backend {
  ...
  impl<'a,> core::fmt::Debug for Httprequest<'a,> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
      f.debug_struct("Httprequest").field("path", &self.path).field("headers", &self.headers).field("querystring", &self.querystring).field("payload", &self.payload).field("method", &self.method).finish()}
  }
  #[derive(Clone)]
  ...
  impl core::fmt::Debug for Httpresponse {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
      f.debug_struct("Httpresponse").field("data", &self.data).field("success", &self.success).finish()}
  }
  ...
  pub struct GuestBackend<T> {
    ...
    handlerequest: wasmtime::TypedFunc<(i32,i32,i32,i32,i32,i32,i32,i32,i32,i32,), (i32,)>,
  ...
  impl<T> GuestBackend<T> {
    ...
    pub fn handlerequest(&self, mut caller: impl wasmtime::AsContextMut<Data = T>,a: Httprequest<'_,>,)-> Result<Httpresponse, wasmtime::Trap> {
      let func_canonical_abi_free = &self.canonical_abi_free;
      let func_cabi_realloc = &self.cabi_realloc;
      let memory = &self.memory;
      let Httprequest{ path:path0, headers:headers0, querystring:querystring0, payload:payload0, method:method0, } = a;
      let vec1 = path0;
      ...
      for (i, e) in vec5.into_iter().enumerate() {
        let base = result5 + (i as i32) * 16;
        {
          ...
          memory.data_mut(&mut caller).store_many(ptr3, vec3.as_bytes())?;
          memory.data_mut(&mut caller).store(base + 4, wit_bindgen_host_wasmtime_rust::rt::as_i32(vec3.len() as i32))?;
          memory.data_mut(&mut caller).store(base + 0, wit_bindgen_host_wasmtime_rust::rt::as_i32(ptr3))?;
          let vec4 = t2_1;
          let ptr4 = func_cabi_realloc.call(&mut caller, (0, 0, 1, vec4.len() as i32))?;
          memory.data_mut(&mut caller).store_many(ptr4, vec4.as_bytes())?;
          memory.data_mut(&mut caller).store(base + 12, wit_bindgen_host_wasmtime_rust::rt::as_i32(vec4.len() as i32))?;
          memory.data_mut(&mut caller).store(base + 8, wit_bindgen_host_wasmtime_rust::rt::as_i32(ptr4))?;
        ...
        memory.data_mut(&mut caller).store_many(ptr7, vec7.as_bytes())?;
        let vec8 = method0;
        let ptr8 = func_cabi_realloc.call(&mut caller, (0, 0, 1, vec8.len() as i32))?;
        memory.data_mut(&mut caller).store_many(ptr8, vec8.as_bytes())?;
        let (result9_0,) = self.handlerequest.call(&mut caller, (ptr1, vec1.len() as i32, result5, len5, ptr6, vec6.len() as i32, ptr7, vec7.len() as i32, ptr8, vec8.len() as i32, ))?;
        let load10 = memory.data_mut(&mut caller).load::<i32>(result9_0 + 0)?;
        let load11 = memory.data_mut(&mut caller).load::<i32>(result9_0 + 4)?;
        let ptr12 = load10;
        let len12 = load11;
        
        let data12 = copy_slice(
        &mut caller,
        memory,
        ptr12, len12, 1,
        )?;
        ...
        Ok(Httpresponse{data:String::from_utf8(data12)
        .map_err(|_| wasmtime::Trap::new("invalid utf-8"))?, success:match i32::from(load13) {
          0 => false,
          1 => true,
